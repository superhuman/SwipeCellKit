...
class SwipeController: NSObject {
    ...
    @objc func handlePan(gesture: UIPanGestureRecognizer) {
        ...
        case .ended, .cancelled, .failed:
            ...
            if let customActionTrigger = self.swipeable?.customActionTrigger {
            ...
            if gesture.state == .cancelled || gesture.state == .failed {
                shouldForceCloseMenu = true
            } else if let forceMenuCloseTrigger = self.swipeable?.forceMenuCloseTrigger {
                switch swipeable.state {
                case .left:
